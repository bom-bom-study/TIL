...
- 다이내믹 프록시 오브젝트는 Proxy의 newProxyInstance()를 통해서만 생성이 가능 => java의 리플렉션을 통한 빈 생성이 아니기 때문에 스프링의 빈으로는 등록할 수 없다
- => 대신 팩토리 빈을 implement해 **getObject()**8메소드에 다이내믹 프록시 오브젝트를 만들어주는 코드를 넣으면 스프링의 빈으로 등록할 수 있다.
